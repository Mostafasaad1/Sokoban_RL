"""
STABLE SOKOBAN PPO TRAINING SYSTEM
===================================

Complete with all stability fixes based on successful training results:
- Stable network architecture with bounded value predictions
- Value loss guards against explosion
...
# =============================================================================
# ULTRA-STABLE NETWORK ARCHITECTURE
# =============================================================================

class StableSokobanNetwork(nn.Module):
    """
    Ultra-stable network with bounded value predictions and conservative architecture.
    Designed specifically to prevent value loss explosion.
    """
    ...
    def __init__(self, obs_shape: Tuple[int, ...], num_actions: int, hidden_size: int = 512):
        ...
        print(f"🧠 STABLE NETWORK ARCHITECTURE:")
        print(f"   Input size: {input_size}")
        print(f"   Hidden size: {hidden_size}")
        print(f"   Actions: {num_actions}")
        
        # Enhanced input normalization for stability
        ...
        # CRITICAL: Conservative value head with bounded output
        ...
        self._initialize_weights_conservative()
        
        total_params = sum(p.numel() for p in self.parameters())
        print(f"   Total parameters: {total_params:,}")
        print(f"   Value output bounded: [-10, 10]")
        
    def _initialize_weights_conservative(self):
        """Ultra-conservative weight initialization for stability"""
    ...
    def forward(self, x):
        ...
        value = self.critic_head(features) * 10.0  # Scale to reasonable range [-10, 10]
        
        return logits, value.squeeze(-1)

# =============================================================================
# STABLE EXPLORATION MANAGER
# =============================================================================

class StableExplorationManager:
    """
    Stable exploration with smooth decay and diversity tracking.
    """
    
    def __init__(self, initial_epsilon=1.0, min_epsilon=0.05, decay_steps=80000):
...
# =============================================================================
# STABLE SOKOBAN ENVIRONMENT
# =============================================================================

class StableSokobanEnv(EfficientSokobanEnv):
    """
    Stable Sokoban environment with scaled rewards to prevent value explosion.
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.scored_rewards_per_step = 0
        
        print("🎮 STABLE SOKOBAN ENVIRONMENT LOADED")
        print("   ✓ Scaled reward shaping")
        print("   ✓ Action diversity tracking")
        print("   ✓ Conservative reward values")
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # 3. TARGET PROGRESS (SCALED DOWN for stability)
        target_progress = current_boxes_on_targets - prev_boxes_on_targets
        if target_progress > 0:
            progress_bonus = target_progress * 10.0  # SCALED DOWN from 30.0
            reward += progress_bonus
            reward_components['target_progress'] = progress_bonus
            self.scored_rewards_per_step += progress_bonus
            ...
            self.scored_rewards_per_step += regression_penalty
        
        # 4. MILESTONE REWARDS (SCALED DOWN for stability)
        if current_boxes_on_targets > self.best_boxes_on_targets:
            ...
            milestone_bonus = 5.0 * (current_boxes_on_targets - old_best)  # SCALED DOWN from 20.0
        ...
        if pos_key not in self.exploration_bonus_used:
            ...
            if len(self.exploration_bonus_used) <= 25:
        ...
        if pushed_box:
            interaction_bonus = 3.5  # SCALED DOWN from 2.5
        ...
        # 8. COMPLETION BONUS (SCALED DOWN for stability)
        if done and self.game.is_solved():
            efficiency_bonus = max(0, 300 - self.step_count) * 1.0  # SCALED DOWN
            completion_bonus = 500.0 + efficiency_bonus  # SCALED DOWN from 350.0
        ...
        if self.step_count >= 300:
            ...
            if not self.game.is_solved():
                ...
                if self.best_boxes_on_targets > 0:
                    timeout_penalty = -1.5 * (4 - self.best_boxes_on_targets)  # SCALED DOWN
                else:
                    timeout_penalty = -8.0  # SCALED DOWN from 12.0
    ...
    def reset(self, seed=None, options=None):
        """Reset environment with tracking"""
...
# =============================================================================
# STABLE CURRICULUM MANAGER
# =============================================================================

class StableCurriculumManager:
    """
    Stable curriculum learning with conservative progression.
    """
    ...
    def __init__(self, success_threshold=0.15, evaluation_window=100):
        ...
        self.min_episodes_per_level = 500
        
        print("📈 STABLE CURRICULUM MANAGER INITIALIZED")
    ...
    def update(self, success: bool, episode_reward: float, boxes_on_targets: int) -> bool:
        ...
        if (len(self.success_history) >= self.evaluation_window and 
            ...
            # Conservative upgrade criteria
            if success_rate >= self.success_threshold and avg_reward > 3.0:
                upgrade_ready = True
                reason = f"success rate ({success_rate:.1%}) and positive reward"
            elif avg_boxes >= 3 and avg_reward > 2.0:
                upgrade_ready = True
                reason = f"box progress ({avg_boxes:.1f} boxes)"
            elif avg_reward > 8.0:  # Good performance
                upgrade_ready = True
            ...
            elif (success_rate < 0.02 and avg_reward < -12.0 and 
...
class StablePPOTrainer:
    """
    Stable PPO training system with all stability fixes.
    """
    ...
    def __init__(self, args):
        ...
        self.envs = [StableSokobanEnv() for _ in range(args.num_envs)]
        ...
        # STABLE OPTIMIZER AND SCHEDULER
        self.optimizer = optim.Adam(self.network.parameters(), lr=args.learning_rate, eps=1e-5)
        self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=500, gamma=0.96)
        
        # Enhanced components
        self.exploration_manager = StableExplorationManager()
        self.curriculum = StableCurriculumManager()
        
        # STABLE PPO PARAMETERS (conservative values)
        self.gamma = 0.98
        ...
        self.value_losses = deque(maxlen=1000)  # Track value loss stability
        
        print(f"\n⚡ STABLE PPO TRAINER INITIALIZED!")
    ...
    def collect_rollouts(self):
        """Collect rollouts with stable exploration"""
        observations = []
        actions = []
        rewards = []
        dones = []
        ...
        for step in range(self.args.num_steps):
            ...
            with torch.no_grad():
                ...
                # Stable exploration: epsilon-greedy + policy sampling
            ...
            for i, env in enumerate(self.envs):
                # if step % 50 == 0:
                #         print(f"🎯Step: {step} recevied rewards +{env.scored_rewards_per_step:.1f}")
    ...
    def update_policy(self, rollouts, advantages, returns):
        ...
        for epoch in range(4):
            ...
            # Guard against value loss explosion (your logs showed 2056 - too high!)
            if value_loss > 1000:
    ...
    def train(self):
        ...
        for update in range(1, num_updates + 1):
            ...
            # Enhanced logging with stability monitoring
            if update % 10 == 0:
            ...
            if update % 100 == 0 and update > 0:
                ...
                filename = f'stable_sokoban_model_{global_step}.pt'
...
def main():
    parser = argparse.ArgumentParser(description='Stable Sokoban PPO Training')
    ...
    parser.add_argument('--learning-rate', type=float, default=1.5e-4, help='Learning rate (reduced for stability)')
    
    args = parser.parse_args()
    
    print("🚀 STABLE SOKOBAN PPO TRAINING SYSTEM")
    print("=" * 50)
    print("✅ STABLE NETWORK with bounded value predictions")
    print("🛡️  VALUE LOSS GUARD against explosion")
    print("📉 CONSERVATIVE HYPERPARAMETERS")
    print("🎯 SCALED REWARDS for stability")
    print("📊 COMPREHENSIVE STABILITY MONITORING")
    print("=" * 50)
    
    try:
        trainer = StablePPOTrainer(args)
        trainer.train()
